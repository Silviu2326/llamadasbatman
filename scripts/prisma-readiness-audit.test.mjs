...
test('el auditor Prisma es offline y no mutante', () => {
  ...
  const migrationCount = fs.readdirSync(path.join(root, 'backend', 'prisma', 'migrations'), { withFileTypes: true })
    .filter((entry) => entry.isDirectory()).length
  assert.ok(report.evidence.some((entry) => entry.migrationCount === migrationCount))
  assert.ok(Array.isArray(report.warnings))
  assert.equal(report.warnings.some((entry) => entry.code === 'DOC_MIGRATION_NOT_FOUND'), false)

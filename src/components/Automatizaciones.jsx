...
const STARTER_TEMPLATES = [
  { Icon: RiPhoneLine, color: '#fb7185', title: 'Seguimiento post-llamada', detail: 'Envía un email y crea una tarea después de cada llamada.', trigger: 'Llamada completada' },
  { Icon: RiUserAddLine, color: '#22d3ee', title: 'Bienvenida a nuevos leads', detail: 'Activa una primera acción cuando entra un contacto nuevo.', trigger: 'Nuevo lead creado' },
  { Icon: RiMailLine, color: '#a78bfa', title: 'Reactivar oportunidades', detail: 'Detecta inactividad y vuelve a abrir la conversación.', trigger: 'Lead sin actividad > 7 días' },
]

function Metric({ Icon, label, value, detail, color }) {
  return <article className="automation-metric"><div className="automation-metric-icon" style={{ color, background: `${color}18`, borderColor: `${color}38` }}><Icon /></div><div><span>{label}</span><strong>{value}</strong><small>{detail}</small></div></article>
}
...
export default function Automatizaciones() {
  ...
  return <div className="dark-scroll automation-page">
    <DataStatusBanner
      status={dataStatus}
      message={loadError || statusMessage(dataStatus, { live: 'Automatizaciones sincronizadas con tu organización.', empty: 'La conexión responde, pero todavía no hay flujos creados.', disconnected: 'No se pueden consultar ni modificar flujos mientras la API está desconectada.' })}

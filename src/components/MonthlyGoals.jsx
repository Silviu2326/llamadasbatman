...
function GoalMetric({ title, value, target, format }) {
  ...
  return <div className="home-month-metric">
    ...
    <p>{progress !== null ? `Objetivo: ${format(target)} · ${progress}%` : 'Sin objetivo definido'}</p>
    {progress !== null ? <progress max="100" value={Math.min(progress, 100)} aria-label={`${title}: ${progress}% del objetivo`} /> : null}
  </div>
}

export function MonthResults({ resource, retry, onSaved, locale, showLinks = true, title = 'Cómo va el mes' }) {
  ...
  return <section className="home-section" aria-labelledby="home-month-title" aria-busy={resource.loading}>
    ...
    {stats ? <>
      ...
      {editing ? <form className="home-goal-form" onSubmit={save}>
        <fieldset disabled={saving}><legend>Objetivos mensuales de la organización</legend>
    ...
    {showLinks ? <div className="home-section-links"><Link className="home-link" to="/plan">Plan y objetivos<RiArrowRightSLine aria-hidden="true" /></Link><Link className="home-link" to="/insights">Análisis del negocio<RiArrowRightSLine aria-hidden="true" /></Link></div> : null}
  </section>
}

...
function MetricCard({ item, value }) {
  ...
  return <article className="calls-metric" style={{ '--metric': item.color }}><div className="calls-metric-icon"><Icon /></div><div><span>{item.label}</span><strong>{value ?? '—'}</strong><small>{value == null ? 'Sin datos disponibles' : 'Datos de la API'}</small></div></article>
}
...
export default function CallsPage() {
  ...
  const runCommand = action => {
    ...
    if (action === 'new') navigate('/voz/test')
  }
  ...
  return <main className="calls-page dark-scroll">
    <header className="calls-header"><div className="calls-title"><div className="calls-title-icon"><RiPhoneLine /></div><div><h1>{t('calls.title')}</h1><p>{t('calls.subtitle')}</p></div></div><div className="calls-header-actions"><button className="calls-command-trigger" onClick={() => setShowCommand(true)}><RiCommandLine /><span>{t('calls.commands')}</span><kbd>⌘ K</kbd></button><button className="calls-button ghost" onClick={loadCalls}><RiRefreshLine /> {t('calls.refresh')}</button><button className="calls-button ghost" onClick={() => setShowFilters(value => !value)}><RiFilter3Line /> {t('calls.filters')}</button><button className="calls-button secondary" onClick={exportCalls}><RiDownload2Line /> {t('calls.export')}</button><button className="calls-button primary" onClick={() => navigate('/voz/test')}><RiPhoneLine /> {t('calls.newCall')}</button></div></header>
    ...
    <DataStatusBanner
      ...
      onAction={dataStatus === 'empty' || dataStatus === 'demo' ? () => navigate('/voz/test') : dataStatus === 'disconnected' ? () => navigate('/configuracion') : undefined}

...
function DataRow({ children, gridTemplate, selected, isLast, onClick, accent }) {
...
export default function DataTable({
  ...
  headerContent, scrollable = true, style,
}) {
    ...
    <div style={{
      ...
      {headerContent ?? (
        <div style={{ display: 'grid', gridTemplateColumns: gridTemplate, gap: 0, padding: '10px 16px', borderBottom: '1px solid #1a2235', flexShrink: 0 }}>
          {normCols.map(col => (
            <span key={col.label} style={{ fontSize: 10, color: col.color ?? '#4b5563', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.4 }}>
              {col.label}
            </span>
          ))}
        </div>

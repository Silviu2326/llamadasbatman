...
export default function LeadsPage() {
  ...
  const kpis = [
    { icon: RiGroupLine, label: 'Leads totales', value: totalLeads.toLocaleString(localeCode(getLocale())), detail: 'Datos de la API', color: '#818cf8' },
    { icon: RiUserAddLine, label: 'Leads nuevos (7 días)', value: stats?.newLeads?.toLocaleString(localeCode(getLocale())) ?? '—', detail: stats ? 'Datos de la API' : 'Sin datos', color: '#22d3ee' },
  ...
  return <div className="dark-scroll leads-page">
    <header className="leads-page-header"><div className="leads-heading"><div className="leads-brand-icon"><RiGroupLine /></div><div><h1>Leads</h1><p>{t('modules.leadsTitle')}</p></div></div><div className="leads-header-actions"><div className="leads-search"><RiSearchLine /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search leads, companies, tags…" /><kbd>⌘ K</kbd></div><button className="leads-button ghost" onClick={() => setRefreshKey(key => key + 1)}><RiRefreshLine /> {t('calls.refresh')}</button><button className="leads-button secondary" onClick={() => setShowImport(true)}><RiDownload2Line /> Importar</button><button className="leads-button primary" onClick={() => setShowNewLead(true)}><RiAddLine /> {t('modal.newLead')}</button></div></header>

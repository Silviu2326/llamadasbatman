...
import { LIFECYCLE, lifecycleOf } from './agents/agentLifecycle'
import './agents-team.css'

// Estilos de estado: la hoja solo trae activo/pausado; borrador y archivado van en línea.
const STATUS_STYLE = { draft: { background: '#eef2fb', color: '#4c618c', borderColor: '#d7dff0' }, archived: { background: '#f1f1f4', color: '#6b7280', borderColor: '#e2e3e8' } }
function StatusBadge({ agent }) {
  const status = lifecycleOf(agent)
  return <span className={`team-status ${status === 'active' ? 'active' : 'paused'}`} style={STATUS_STYLE[status]} title={LIFECYCLE[status].description}><i style={status === 'draft' || status === 'archived' ? { background: STATUS_STYLE[status].color } : undefined} />{LIFECYCLE[status].label}</span>
}
...
export default function AgentsTeam() {
  ...
  const rows = useMemo(() => (agents || []).filter(agent => `${agent.name} ${agent.role || ''}`.toLocaleLowerCase().includes(search.toLocaleLowerCase()) && (status === 'all' || lifecycleOf(agent) === status)).sort((a, b) => sort === 'name' ? a.name.localeCompare(b.name, 'es') : new Date(b.createdAt) - new Date(a.createdAt)), [agents, search, status, sort])
  ...
  return <section className="agents-team-page">
    ...
    <section className="team-summary team-panel" aria-label="Resumen del equipo"><div className="team-summary-main"><h2>Resumen de {dayTitle}</h2><div className="team-kpis">
      <div className="team-kpi"><span className="team-kpi-main"><i className="team-dot" /><strong>{agents ? agents.filter(agent => lifecycleOf(agent) === 'active').length : '—'}</strong> agentes publicados</span><small>de {agents?.length ?? '—'} agentes{agents?.some(agent => lifecycleOf(agent) === 'draft') ? ` · ${agents.filter(agent => lifecycleOf(agent) === 'draft').length} en borrador` : ''}</small></div>
    ...
    <div className="team-toolbar"><div className="team-tabs" role="tablist" aria-label="Vista del equipo"><button id="team-agents-tab" role="tab" aria-selected={view === 'agents'} aria-controls="team-view" onClick={() => setView('agents')}>Agentes ({agents?.length ?? '—'})</button><button id="team-performance-tab" role="tab" aria-selected={view === 'performance'} aria-controls="team-view" onClick={() => setView('performance')}>Rendimiento</button></div><div className="team-filters"><label className="team-search"><RiSearchLine /><input aria-label="Buscar agente" placeholder="Buscar agente…" value={search} onChange={event => setSearch(event.target.value)} /></label><select aria-label="Estado del agente" value={status} onChange={event => setStatus(event.target.value)}><option value="all">Todos los estados</option><option value="active">Activos</option><option value="draft">Borradores</option><option value="paused">Pausados</option><option value="archived">Archivados</option></select><select aria-label="Ordenar agentes" value={sort} onChange={event => setSort(event.target.value)}><option value="recent">Más recientes</option><option value="name">Nombre A–Z</option></select></div></div>
    <section id="team-view" className="team-panel team-table-panel" role="tabpanel" aria-labelledby={view === 'agents' ? 'team-agents-tab' : 'team-performance-tab'}>
      {view === 'agents' ? <div className="team-table-scroll"><table className="team-table"><thead><tr><th rowSpan="2" scope="col">Agente</th><th rowSpan="2" scope="col">Última actividad</th><th colSpan="3" scope="colgroup" className="team-today">Hoy</th><th rowSpan="2" scope="col">Resultados</th><th rowSpan="2" scope="col">Objetivo</th><th rowSpan="2" scope="col">Acciones</th></tr><tr className="team-subhead"><th scope="col">Llamadas</th><th scope="col">Conversaciones</th><th scope="col">Reuniones</th></tr></thead><tbody>
        {rows.map(agent => { const metric = stats(agent.id); const call = latest[agent.id]; const goal = Number(agent.settings?.dailyMeetingGoal) || 0; const progress = metric && goal ? Math.min(100, Math.round(metric.meetings / goal * 100)) : 0; return <tr key={agent.id}>
          <td><div className="team-identity"><Avatar agent={agent} /><div><div className="team-name-line"><button className="team-name" onClick={() => openAgent(agent)}>{agent.name}</button><StatusBadge agent={agent} /></div><p>{agent.role || 'Agente de voz'}</p><small>{agent.callDirection === 'inbound' ? 'Recibe y atiende llamadas.' : agent.callDirection === 'both' ? 'Llamadas entrantes y salientes.' : 'Realiza llamadas salientes.'}</small></div></div></td>
          ...
          <td><div className="team-row-actions"><button className="team-button compact" onClick={() => openAgent(agent)} aria-label={`Abrir ${agent.name}`}><RiExternalLinkLine />Abrir</button><button className="team-button compact" onClick={() => setScheduleAgent(agent)} aria-label={`Programar una llamada con ${agent.name}`}><RiCalendarLine />Programar</button><button className="team-button primary compact" onClick={() => navigate(`/agentes/${agent.id}?section=quality`)} aria-label={`Prueba real de ${agent.name}`} title="Llamada real a un número propio autorizado, grabada y evaluada."><RiPlayFill />Prueba real</button></div></td>
        </tr> })}

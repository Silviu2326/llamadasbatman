...
export default function CallDetailPage() {
  ...
  const toggleFavorite = async () => { const response = await apiFetch(`/api/calls/${id}/favorite`, { method: 'POST' }); if (response.ok) { const data = await response.json(); setStarred(Boolean(data.isFavorite)) } else setError('No se pudo actualizar favoritos.') }
  const addNote = async () => { if (!newNote.trim()) return; const response = await apiFetch(`/api/calls/${id}/notes`, { method: 'POST', body: JSON.stringify({ text: newNote.trim() }) }); if (!response.ok) { setError('No se pudo guardar la nota.'); return }; const created = await response.json(); setNotes(list => [created, ...list]); setNewNote('') }
  const deleteNote = async noteId => { const response = await apiFetch(`/api/calls/${id}/notes/${noteId}`, { method: 'DELETE' }); if (response.ok) setNotes(list => list.filter(item => item.id !== noteId)); else setError('No se pudo eliminar la nota.') }
  const toggleTask = async task => { const response = await apiFetch(`/api/calls/${id}/tasks/${task.id}`, { method: 'PUT', body: JSON.stringify({ done: !task.done }) }); if (response.ok) setTasks(list => list.map(item => item.id === task.id ? { ...item, done: !item.done } : item)); else setError('No se pudo actualizar la acción.') }
  ...
  if (loading) return <div className="detail-loading"><span>{t('details.loadingCall')}</span></div>
  ...
  return <main className="call-detail-page dark-scroll"><div className="detail-topbar"><button className="detail-back" onClick={() => navigate(`/llamadas${window.location.search}`)}><RiArrowLeftLine /> Volver a llamadas</button><div className="detail-top-actions"><button className={starred ? 'is-starred' : ''} onClick={toggleFavorite}>{starred ? <RiStarFill /> : <RiStarLine />} {starred ? 'Destacada' : 'Destacar'}</button><button onClick={copySummary}><RiFileCopyLine /> Copiar resumen</button><button onClick={() => setTab('Notas')}><RiEdit2Line /> Añadir nota</button></div></div>

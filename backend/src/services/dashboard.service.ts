...
export async function getStats(orgId: string) {
  ...
  const timeSeries = days.map(day => ({
    ...
    llamadas: recentCalls.filter(c => c.createdAt.toISOString().slice(0, 10) === day).length,
    reuniones: recentMeetings.filter(m => m.createdAt.toISOString().slice(0, 10) === day).length,
    contactados: 0,
    conversion: 0,
  }))
